...
@section('content')
    ...
    <section class="row">
    	<div class="col-xs-12 banner_img">
            <img src="{{url('/')}}{{'/'}}{{ ThemeService::path('assets/tpso/images/banner_download.jpg') }}">
        </div>
    </section>
                ...
                <div class="col-xs-12 wrap_item_download">
                    @foreach ($data['data']['data'] as $dataitem)
                        <div class="item_download">{{$dataitem['title']}}
                            <div class="wrap_btn_download"><a class="btn_view" href="#"><img src="{{url('/')}}{{'/'}}{{ ThemeService::path('assets/tpso/images/eye.svg') }}">view</a><a class="btn_download" href="#"><img src="{{url('/')}}{{'/'}}{{ ThemeService::path('assets/tpso/images/icon_download.svg') }}">download</a></div>
                        </div>
                    @endforeach
                </div>
            </div>
    ...
    <script>
        ...
        $("body").on('change', '.selectdownload', function() {
            ...
            window.location.href = pathname.replace(/[0-9]/g, pk) ;
