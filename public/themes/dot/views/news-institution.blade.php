            ...
            <div class="container">
                <div class="row">
                    <div class="col-xs-12 head_cshare_detail">
                        <hgroup>
                            <h2>{{__('menu_news_agency')}}</h2>
                            <h1>NEWS INSTITUTION</h1>
                        </hgroup>
                    </div>
                </div>
            </div>
            ...
            @if(!empty($data))
                ...
                <div class="border_listdownload">
                    <div class="listdownload_item">
                        <a href="{{route('news.institution.detail',['id'=>$val['id']])}}">
                        <div class="listdownload_head">
                            {{$val['title']}}
                        </div></a>
                        <div class="listdownload_btn"><a href="{{route('news.institution.download',['id' => $val['id']])}}" target="_blank">
                        {{__('download')}}
                        </a>
                        </div>
                    </div>
...
$(document).ready(function(){
    $('title').text('{{__('menu_news_agency')}}');
});
</script>
@endpush