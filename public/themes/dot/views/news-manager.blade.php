    ...
    <div class="container">
        <div class="row">
            ...
            <a href="{{url('/')}}">{{__('menu_home')}}</a>
                ...
                <span>{{__('menu_executive_news')}}</span>
            </div>
        </div>
    </div>
</section>
...
<section class="row wow fadeInDown">
    ...
    @if(!empty($alldata))
    <div class="row row_viewcolumn" style="margin:10px;">
    @foreach($alldata as $key => $val)
        ...
        @endphp
            ...
            <a href="{{route('news.manager.detail',['id'=>$val['id']])}}" class="col-xs-12 col-sm-12 col-md-12 item_viewcolumn">
                <figure>
                @if(!empty($val[0]['cover_image']))
