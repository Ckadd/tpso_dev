...
@section('content')
    <div class="page-content read container-fluid">
            ...
            <div class="col-md-12">
                ...
                <div class="panel panel-bordered" style="padding-bottom:5px;">
                    ...
                    @foreach($dataType->readRows as $row)
                        @php
                            $rowDetails = json_encode($row->details);
                            if ($rowDetails === null) {
        ...
        <!-- Begin show detail in formgenerate -->
        <div class="row">
            <div class="col-xs-12">
                <div class="panel panel-bordered">
                    <div class="panel-heading">
                        <h3 class="panel-title panel-icon"><i class="voyager-window-list"></i>{{ __('voyager::generic.Detail_in_formgenerate') ." " . __('voyager::generic.sum_post_formgenerate')." " .FormGenerateService::sumDataDetailInFormgenerate($dataTypeContent->id) ." ".__('voyager::generic.form')}} </h3>
                    </div>
                        ...
                        <table id="dataTable" class="table table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <th>{{ __('voyager::generic.formtype') }}</th>
                                    <th>{{ __('voyager::generic.formname') }}</th>
                                    <th>{{ __('voyager::generic.formvalue') }}</th>
                                    <th></th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach (FormGenerateService::findDetailInFormgenerate($dataTypeContent->id)['form_detail'] as $key => $valDetail)
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    <td>{{ $valDetail['form_id'] }}</td>
                                    <td>{{ $valDetail['type'] }}</td>
                                    <td>{{ $valDetail['name'] }}</td>
                                    <td>{{ $valDetail['value'] }}</td>
                                    <td id="bread-actions" style="">
                                        <a class="btn btn-sm btn-primary pull-right edit" href="{{ route('voyager.form-generate-details.edit', ['id' => $valDetail['id']]) }}">
                                            <i class="voyager-edit"></i>
                                            <span class="hidden-xs hidden-sm">{{ __('voyager::generic.edit') }}</span>
                                        </a>
                                        <a class="btn btn-sm btn-warning pull-right view" style="margin-left: 5px;" href="{{ route('voyager.form-generate-details.show', ['id' => $valDetail['id']]) }}">
                                            <i class="voyager-edit"></i>
