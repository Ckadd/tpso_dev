                            ...
                            @foreach($dataTypeRows as $row)
                                ...
                                @else
                                    <div class="form-group @if($row->type == 'hidden') hidden @endif col-md-{{ isset($display_options->width) ? $display_options->width : 12 }}" @if(isset($display_options->id)){{ "id=$display_options->id" }}@endif>
                                        ...
                                        @php 
                                            $resolution = ($row->type == 'image') ? 'padding-left:5px; color:red;' : 'display:none'; 
                                        @endphp
                                        <label for="name">{{ $row->display_name }}</label><label style='{{$resolution}}'>Resolution 373 × 142 (หน่วยงานภายใน 223 x 224)</label>
